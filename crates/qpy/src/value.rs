...
pub(crate) fn get_circuit_type_key(op: &PackedOperation) -> PyResult<CircuitInstructionType> {
    match op.view() {
        ...
        OperationRef::Opaque(opaque_operation) => match opaque_operation {
            qiskit_circuit::custom_operations::OpaqueOperation::Gate(_) => Ok(CircuitInstructionType::Gate),
            qiskit_circuit::custom_operations::OpaqueOperation::Instruction(_) => Ok(CircuitInstructionType::Instruction),
        },
        ...
        OperationRef::CustomOperation(custom_operation) => Err(PyErr::new::<PyValueError, _>(format!(
                    "Unable to determine circuit type key for {:?}",
