...
pub fn analyze_commutations(
...
) -> Result<(CommutationSet, NodeIndices), CommutationAnalysisError> {
    ...
    let evaluate_qubit =
        |qubit: usize,
         ...
         -> Result<(), CommutationAnalysisError> {
    ...
    if qiskit_util::getenv_use_multiple_threads() {
        commutation_set
            ...
            .enumerate()
            .map(
                |(qubit, (local_commutation_set, local_indices))| -> Result<(), CommutationAnalysisError> {
                    evaluate_qubit(qubit, local_commutation_set, local_indices)?;
                    Ok(())
                },
            )
            .collect::<Result<(), CommutationAnalysisError>>()?;
