...
pub enum ConsolidateBlocksError {
    ...
    PyMatrixError(PyErr),
    #[error("{0}")]
    TwoQubitBasisDecomposer(String),
}

impl From<ConsolidateBlocksError> for PyErr {
    fn from(value: ConsolidateBlocksError) -> Self {
        match value {
            ConsolidateBlocksError::DAGCircuit(dagcircuit_inner_error) => dagcircuit_inner_error.into(),
            ...
            ConsolidateBlocksError::PyMatrixError(py_err) => py_err,
            ConsolidateBlocksError::TwoQubitBasisDecomposer(_) => todo!(),
        }
    }
}
...
fn inner_run_consolidate_blocks(
    ...
    let valid_op_node = |dag: &mut DAGCircuit, index: usize| -> Result<NodeIndex, ConsolidateBlocksError> {
                    ...
                    let num_basis_gates = if let Some(ref decomposer) = decomposer {
                        match decomposer {
                            DecomposerType::TwoQubitBasis(decomp) => {
                                ...
                                >(
                                    matrix.as_view()
                                )).map_err(|err| ConsolidateBlocksError::TwoQubitBasisDecomposer(format!("{:?}", err)))?
                            }
