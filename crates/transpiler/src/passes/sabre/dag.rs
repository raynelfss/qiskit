...
use qiskit_circuit::dag_circuit::{DAGCircuit, DAGCircuitInnerError, NodeType, Wire};
...
impl InteractionKind {
    fn from_control_flow(cf: ControlFlowView<DAGCircuit>) -> Result<Self, DAGCircuitInnerError> {
        let blocks: Box<[_]> = cf
            .blocks()
            .into_iter()
            .map(|dag| Ok((SabreDAG::from_dag(dag)?, dag.clone())))
            .collect::<Result<_, DAGCircuitInnerError>>()?;
        Ok(Self::ControlFlow(blocks))
    }

    fn from_op(op: &PackedOperation, qargs: &[Qubit]) -> Result<Self, DAGCircuitInnerError> {
        if op.directive() {
...
impl SabreDAG {
    pub fn from_dag(dag: &DAGCircuit) -> Result<Self, DAGCircuitInnerError> {
