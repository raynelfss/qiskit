...
fn extract_definition(op: &PackedOperation, params: &[Param]) -> PyResult<Option<CircuitData>> {
    match op.view() {
        ...
        OperationRef::ControlFlow(_) | OperationRef::Operation(_) | OperationRef::Opaque(_) | OperationRef::CustomOperation(_) => Ok(None),
        OperationRef::CustomGate(custom_gate) => Ok(custom_gate.definition(params)),
        OperationRef::CustomInstruction(custom_instruction) => Ok(custom_instruction.definition(params)),
            }
}
...
fn synthesize_op_using_plugins(
    ...
    let op_py = match op {
        ...
        OperationRef::Opaque(opaque_operation) => opaque_operation.create_py_op(py, Some(params.iter().cloned().collect()))?,
        OperationRef::CustomGate(custom_gate) => custom_gate.create_py_op(py, Some(params.iter().cloned().collect()))?.unbind(),
        OperationRef::CustomInstruction(custom_instruction) => custom_instruction.create_py_op(py, Some(params.iter().cloned().collect()))?.unbind(),
        OperationRef::CustomOperation(custom_operation) => custom_operation.create_py_op(py, Some(params.iter().cloned().collect()))?.unbind(),
    };
