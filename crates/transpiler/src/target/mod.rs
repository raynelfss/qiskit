        ...
        if let Some(qubit_properties) = qubit_properties.as_ref() {
            if num_qubits.is_some_and(|num_qubits| num_qubits > 0) {
                if num_qubits.unwrap() as usize != qubit_properties.len() {
                    return Err(TargetError::NumQubitMisMatch { num_qubits: num_qubits.unwrap(), num_props: qubit_properties.len() });
                }
            } else {
...
impl PyTarget {
    ...
    fn py_add_instruction(
        ...
        let new_name: String = match &instruction {
            ...
            TargetOperation::Variadic(_) => {
                if name.is_none() {
                    return Err(TranspilerError::new_err("A name must be specified when defining a supported global operation by class."))
                }
                if properties.is_some() {
                    return Err(TranspilerError::new_err("An instruction added globally by class can't have properties set."))
                }
                name.unwrap().to_string()
            },
        };
        let properties =
        if properties.is_none() || instruction.is_variadic() {
           [(None::<String>, None::<InstructionProperties>)].into_py_dict(py)?.unbind()
        } else {
            properties.unwrap()
        };
    ...
    fn py_update_instruction_properties(
        ...
        self.inner.update_instruction_properties(&instruction, &qargs, properties.clone())
            .map_err(|err| PyKeyError::new_err(err.to_string()))?;
        self.gate_map.bind(py).get_item(&instruction)?.unwrap().set_item(&qargs, properties)
    }
    ...
    fn py_operation_names<'py>(&'py self, py: Python<'py>) -> Bound<'py, PyList> {
    ...
    fn __setstate__(&mut self, state: Bound<PyDict>) -> PyResult<()> {
        ...
        for (gate, bounds) in bounds_map {
            self.inner.add_owned_angle_bound(gate.as_str(), bounds)
